...
const PanWrapper: FunctionComponent<Props> = (props) => {
  ...
  const [currentPosition, setCurrentPosition] = useState<ICoordinate>(
    new Coordinate(0,0)
  );
  ...
  useEffect(() => {
    Animated.timing(scale, {
      toValue: store.zoomed ? props.zoomScale : 1,
      duration: 300,
  ...
  useEffect(() => {
    if (store.selectedNode.coordinate != undefined) {
      Animated.timing(pan, {
        toValue: store.zoomed
          ? store.selectedNode.coordinate.scaledCartesian(
            props.zoomScale,
            props.size
          )
          : store.selectedNode.coordinate.cartesian(
            props.size
          ),
        duration: 300,
        useNativeDriver: false,
